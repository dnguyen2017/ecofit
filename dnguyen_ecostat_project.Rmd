...
m3 <- gls(l_next_total ~ l_total*ftreatment -1, 
    data = weevil, 
...
AIC(pop_gls_none, m3, pop_lme_trt) %>%
...
# std_resid is the ratio of group-specific sd that is reported by summary(m3)
var_est_trt <-
  augment(m3) %>%
  group_by(ftreatment) %>%
...
m3_int$coef %>%
  ...
  mutate(parameter = case_when(str_detect(coefficient, "l_total") ~ "b",
