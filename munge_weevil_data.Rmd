...
for (i in seq_along(sheet_pops)) {
  ...
  df_long <- df_now %>%
    select(-pellets,-noclue,-m_added,-f_added) %>% # always 2 pellets a day, and drop noclue as well as the num. added
    filter(!is.na(days)) %>%  # drop empty bottom two rows (included when reading due to sum(pellets) in spread sheet)
    pivot_longer(
      -c(days, date, treatment),
      names_sep = "_",
...
```{r}
full_df %>%
  ggplot(aes(x = date, y = total, group = replicate, col = replicate)) +
  geom_line() + 
  facet_wrap(~treatment) +
  theme_light() +
  theme(legend.position = "none") +
