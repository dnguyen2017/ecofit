...
lh_data %>%
  ...
  group_by(pct_shell, gender) %>%
  mutate(mean_surv = mean(daily_surv)) %>%
  # group_by(pct_shell, gender) %>%
  # mutate(mean_lifetime = mean(lifetime)) %>%
  # ungroup() %>%
  ggplot() +
...
The top two models include sex and percent peanut shell as covariates.
...
### Thoughts
It is clear that the sex of adult weevils has an effect on survival and that resource quality doesn't much of an effect on survival. We should expect that females will have lower survival than males since they allocate more resources to reproduction. However, I think it is interesting that resource quality has such a weak effect on survival since weevils don't require food after emerging as adults so adult weevils rely almost entirely on what they ingested as larvae.
...
Using these vital rate estimates I can construct population projection matrices.
