...
surv_data %>%  
  ...
  labs(title = "Weevil daily survival probability",
       subtitle = "Survival probality is assumed to be constant (exponential distributed lifetimes).",
       x = "Percent peanut shell in bean",
...
Not sure how "real" The effect of percent peanut shell on survival is. I think it may be driven by just a few influential observations that were unusually low for both males and females (see plots of observed data). It's not clear to me that these low observations are attributable to food quality. This is reflected in the relative AIC weights for the models; the S ~ sex model has 0.71 support from the data and the S ~ sex + % shell model has 0.29 support from the data.
...
sexshell.predict$estimates )#, averaged.predict$estimates)
names(pred_list) <- c("survival ~ intercept", "survival ~ shell", "survival ~ sex", "survival ~ sex + shell")#, "averaged")
...
It is clear that the sex of adult weevils has an effect on survival and that resource quality doesn't much of an effect on survival. 
