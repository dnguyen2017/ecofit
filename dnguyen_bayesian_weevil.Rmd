...
```{r, out.width = "500px"}
knitr::include_graphics("pics/dag_direct_density.png")
```
...
* $\beta_{ij}:$  effect of previous population size at $t-1$ on expected number of adult weevils at time $t$ for treatment $i$ in replicate $j$
* $\mu_{\alpha,i}:$ the mean population growth rate for treatment $i$
* $\mu_{\beta_{i}}:$ the mean effect of previous population size at $t-1$ on expected number of adult weevils at time $t$ for treatment $i$
...
[\boldsymbol{\alpha, \beta, \mu_{\alpha}, \mu_{\beta}},Z, \sigma_{p}^2, \sigma_{\alpha}^2, \sigma_{\beta}^2| \boldsymbol{N}] \propto  
                                                    \prod_{i=1}^{4} \prod_{j=1}^{J_{ij}} \prod_{t=1}^{T_{ij}} 
                                                  & \text{Poisson}[n_{ij,t} | \exp(Z_{ij,t})]\\
                                                  & \text{normal}(Z_{ij,t} | g(\alpha_{i},\beta_{ik},n_{ij,t-1}, \ldots, n_{ij,t-1}), \sigma_p^2)\\
                                                  & \text{N}[\alpha_{ij} | \mu_{\alpha,i}, \sigma_{\alpha}^2] \\
                                                  & \text{N}[\beta_{ij} | \mu_{\beta_{i}}, \sigma_{\beta}^2] \\
                                                  ...
                                                  & \text{inv.gamma}[\sigma_{p}^2 | 0.001, 0.001] \\
\end{align}
...
```{r echo = TRUE}
MCMCvis::MCMCsummary(cs, params = c("mu_a","mu_b", "sd_a","sd_b", "sd_p", "K")) %>% knitr::kable(digits = 2)
#lapply(cs, coda::effectiveSize) %>% dplyr::bind_rows() #%>% summarise_all(funs(sum))# can also comput n.eff like this
```
...
tibble(treatment = c(0, 0.05, 0.1, 0.15),
  ...
  knitr::kable(caption = "Posterior predictive check: coefficient of variation")
```


Maximum: $\text{max}(\boldsymbol{n_i})$
...
tibble(treatment = c(0, 0.05, 0.1, 0.15),
  ...
  knitr::kable(caption = "Posterior predictive check: max abundance")
```


# Comparing models

$X_{ij,t} = a_i + b_i X_{ij,t-1}+N(0,\sigma_i^2)$, for treatment $i = 1,2,3,4$, replicate $j = 1,\ldots,10$.
...
```{r, out.width = "700px", fig.cap="Log-Gompertz model estimated by GLS"}
