...
weevil %>%
  ...
  labs(title = "Bean weevil abundances",
...
[\boldsymbol{\alpha, \beta, \mu_{\alpha}, \mu_{\beta}},Z, \sigma_{p}^2, \sigma_{\alpha}^2, \sigma_{\beta}^2| \boldsymbol{N}] \propto  
                                                  ...
                                                  & \text{N}[\mu_{\alpha,i} | 0, 100] \\
                                                  & \text{N}[\mu_{\beta_{ik}} | 0, 100] \\
...
variablenames <- c("mu_a","mu_b", "sd_a","sd_b")
modelstring="
model{
  #likelihood
  for (t in 1:length(n)){
  ...
  for (j in 1:length(sreplicate)) {
      a[i, j] ~ dnorm(mu_a[i], 1/sd_a)
      b[i, j] ~ dnorm(mu_b[i], 1/sd_b)
  }
  }
  ...
  # prior for sd
  sd_a ~ dgamma(0.001, 0.001)
  sd_b ~ dgamma(0.001, 0.001)
}"

jm = jags.model(textConnection(modelstring),data,inits,n.chains=length(inits),n.adapt=1000) #create model in R
update(jm, n.iter=10000) #burn-in
cs = coda.samples(jm,variable.names = variablenames,n.iter=10000) #generate chains
#save(cs, file = "chains/weevil_random_trt_no_proc_weird.RData")
``` 
...
This is the full model presented earlier with direct density dependence only. It differs from the model 2 since I include process error.
...
model{
  ...
  tau_p ~ dunif(0, 0.5) # same process error across treatments
...
update(jm, n.iter=10000) #burn-in
cs = coda.samples(jm,variable.names = variablenames,n.iter=10000) #generate chains
#save(cs, file = "chains/weevil_random_trt_proc_error_weird.RData")
``` 

The first time I fit the full model, I accidently misspecified the precision as the standard error. Other than having "sd" estimates that were wrong, the fit seemed fine (all plots on left hand side). I fixed the error and estimated sd correctly. However, now the chains have really odd behavior (plots on right hand side). I'm not sure why there would be such a massive difference from run-to-run.

```{r out.width="50%", fig.show="hold"}
load("chains/weevil_random_trt_proc_error_weird.RData")
cs_weird <- cs
...
```{r out.width="50%", fig.show="hold"}
gelman.plot(cs)
gelman.plot(cs_weird)
```

On one of the runs, the chains appeard to converge (left) although there was still autocorrelation in the chains. However, on another run, it doesn't even look like the chains converged (right) and the chains were getting stuck.

```{r out.width="50%", fig.show="hold"}
mcmc_dens_overlay(cs)
mcmc_dens_overlay(cs_weird)
```
...
```{r}
MCMCsummary(cs) %>% knitr::kable(digits = 2)
MCMCsummary(cs_weird) %>% knitr::kable(digits = 2)
```
